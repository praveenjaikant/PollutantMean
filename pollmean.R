pollutantmean <- function(directory = "C:/Praveen/MS/Data Science Material/Assignments/specdata/", pollutant, id = 1:332) {
  ...
  for (i in 1:id)
  {
    ...
    if (i <10) { 
      data <- read.csv(paste(directory,"0","0", as.character(i), ".csv", sep=""),  ## for example, if 'id' =7, we get 007.csv
                       header = T, 
                       na.strings=c("NA","NaN", " "))
    }
    
    else if (i>=10 & i<100) { 
      data <- read.csv(paste(directory,"0", as.character(i), ".csv", sep=""),  ## for example, if 'id' = 17, we get 017.csv
                       header = T, 
                       na.strings=c("NA","NaN", " ") 
      )
    }
      ...
      data <- read.csv(paste(directory,as.character(i), ".csv", sep=""),     ## Normal
                       header = T, 
                       na.strings=c("NA","NaN", " ") 
      )
    }
